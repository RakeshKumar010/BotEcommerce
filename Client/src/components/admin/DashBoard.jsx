...
const DashBoard = () => {
  ...
  const cardData = [
    { name: "Products", value: product, icon: <FaSitemap className="text-black text-2xl" /> },
    { name: "Admin", value: admin, icon: <MdAdminPanelSettings className="text-black text-2xl" /> },
    ...
    { name: "Coupon", value: coupon, icon: <RiCoupon3Fill className="text-black text-2xl" /> }
  ];

  useEffect(() => {
    const getFun = async () => {
      let result1 = await fetch("https://botecommerce.onrender.com/product");
      result1 = await result1.json();
      setProduct(result1.length);

      let result2 = await fetch("https://botecommerce.onrender.com/coupon");
      result2 = await result2.json();
      setAdmin(result2.length);

      let result3 = await fetch("https://botecommerce.onrender.com/admins");
